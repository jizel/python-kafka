...
def load_config(path: Path = DEFAULT_CONFIG_PATH) -> Dict[str, Any]:
    """Load JSON configuration from ``path`` if it exists.

    The config file is optional; when present it can provide defaults for
    connection details such as ``server``, ``username``, and ``password``.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Path to JSON config file with server/username/password",
    )
    parser.add_argument(
        "--bootstrap-servers",
        default=None,
    ...
    args.bootstrap_servers = args.bootstrap_servers or config.get("server")
    args.topic = args.topic or config.get("topic", "migration-requests")
    args.output = args.output or config.get("output", "-")
    args.security_protocol = args.security_protocol or config.get("security_protocol", "PLAINTEXT")
...
def main() -> None:
    ...
    if not args.bootstrap_servers:
        raise SystemExit(
            "Kafka bootstrap servers are required; add them to the config file "
            "or pass --bootstrap-servers"
        )
